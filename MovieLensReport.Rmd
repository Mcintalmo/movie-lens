...
(https://movielens.org). The data set presented in the analysis to follow
includes 10 million user ratings. The raw data set is not that useful. A human
could never look at that quantity of information and draw many valuable
conclusions.
However, through the application of data science there are
many insights to be had and tools to be built For example, a movie recommendation
system.

The goal of this projects is to develop a system that can predict how a user
will rate a movie. The metric by which to measure success will be the Root Mean
Squared Error, or RMSE, between how the model predicts a user will rate a movie 
and how the user actually rated the movie. Success is considered to be a model 
that results in an RMSE of less than 0.86490.

The key steps, to follow, are preparation, exploration, determination of the
model, and validating the set to find the final RMSE. Preparing the data will
involve downloading, cleaning, and tidying the data set. Exploring the data will 
...
The following analysis includes function from the tidyverse, caret, data.table,
lubridate, gridExtra, and randomForest packages. Significant digits have been
set to 5 to check proximity to the target RMSE, 0.86490.
...
With the file the downloaded, a movies data frame will hold the information with
columns: userId(int), movieId(int), rating(float), timestamp(int), title(string),
genres(string, separated by |). Further manipulation of this dataset will occur
later.
```{r, results = "hide"}
...
The following code ensures that userId and movieId in validation set are also in 
edx set.
...
Because of the length of the process, save and load are utilized to save data
that took a long time to produce, for example, the edx and validation sets.
...
dim(edx)
```
From this, we see that the timestamp is saved as an int (the number of seconds
since New Years 1970). The year a movie is released is included in its title.
Genres are a string separated by |'s. There are 9000055 observations in the set,
...
edx %>%
  ...
  annotate("text", x = mean(edx$rating), y = 2.5 * 10 ^ 6, label = "Average\n", color = "red", angle = 90)
...
at 5:02 AM. Note that the date was saved a column in the data set, as it is often
referenced and takes some time to run.


For a similar reason, a column is added to the edx data set to store the week.
Below is a graph indicating the number of ratings the website received each week.
Despite a week being an arbiturary unit to round to, it was valuable in building
the model, show later.
```{r}
edx <- edx %>%
...
attracted to the site to submit these reviews? Data seems otherwise evenly 
distributed.
...
edx %>%
  ...
  geom_line()
```
Most reviews within a few years of release. There also exist some 
...
This one is harder to track down. Ratings early in the existence of MovieLens
skew very high. Likely because people interested in rating movies, and such
that they would seek out new movie rating databases, are more likely to be
interested in movies and thus rate them higher. Within a few month, the data
fall back to a pattern. There is not much distance from the mean.
With the exception of 1997 to 1999, the data appears
to be oddly periodic.
...
edx %>%
  ...
  geom_smooth(color = "red")
```
Here is revealed a near half-star increase in average rating for movies released around 1960. 
However, being released after about 1985 seems to have a largely negative impact on average
ratings for that year. Are we seeing a nostalgia effect? Is it that the movies users
are still watching from the 1950 must be better? Does it have to do with the 
...
predictive model. We assume ratings from other users, the user's ratings of other
...
Because later the effects will be regularized, and because an estimation of the
final RMSE is important, the edx set is split into a train_set and a test_set.

This will be done by creating a data partition with the caret package.

%20 of the edx data go to the test set.

For reproduceability, I will set the seed to 1 (no rounding).
```{r}
set.seed(1)
...
to determine the effectiveness of the model. The goal is the minimize this
value. Overtraining is undesirable, as it will not translate to the final validation.
An RMSE of less than 0.86490 on the validation set is considered success.
...
line so that improvements of the model are easier to gauge.

This is based on 34.7.4 A First Model
```{r}
...
will have an impact on average ratings of a movie. But much of this effect may
have been hidden within movie and user effects, so it is important to confirm
the RMSE drops sufficiently with the addition of another effect. There are 
diminishing returns for each effect, so finding the right few to represent the
model and draw conclusions is important.

### Combined Genres
The genre data were presented as a string separated with |'s. The simplest model
would be to assume that each combination of genres can be considered its own 
genre. That is, let us briefly assume that action|comedy and action|drama are 
completely orthoganal predictors.

We could then build this model, following the approach of the movie and user effects.
...
been what might seem intuitive. They have also been categorical.

But we are provided far more data than movie,
user, and genre in a rating. We saw in our exploration that there seems to be
...
We saw in exploration that most weeks have plenty of reviews, so again we
will group the data by the week of the review.
```{r}
week_avgs <- train_set %>%
  left_join(movie_avgs, by = "movieId") %>%
  left_join(user_avgs, by = "userId") %>%
...
week_avgs %>%
  ggplot(aes(x = week)) +
  ylim(-0.15, 0.15) +
  geom_point(aes(y = b_d), alpha = 0.25) +
  ...
  scale_colour_manual(name = "Fit",
      values = c(glm = "red", knn = "blue", rpart = "purple", rf = "orange"))
```
```{r echo = FALSE, results = "hide"}
...
year_avgs %>%
  ...
  ylim(-0.1, 0.2) +
  geom_line(aes(y = b_y, color = "observed"), size = 0.75) + 
  geom_line(aes(y = fit_loess, color = "loess")) +
  geom_line(aes(y = fit_knn, color = "knn")) +
  geom_line(aes(y = fit_rf, color = "rf"), size = 0.65) +
...
Historically, only the random forest method has been the best at producing a
...
rounding_rmse <- sapply(rounding_factor, function(r){
...
min(rounded)
rounding_factor[which.min(rounded)]
qplot(rounding_factor, rounding_rmse)
```
...
We can correct for that by going back through our model and adding a lambda
value to the denominator of the mean. That is, instead of dividing by the number
...
target RMSE. Over-training is a concern, because of the number of factors used 
and the use of 
...
However, it is outside the scope of this analysis to exhaust exploration of all 
effects.
...
From there, enough effects could be applied to the model until over-fitting
is observed.
